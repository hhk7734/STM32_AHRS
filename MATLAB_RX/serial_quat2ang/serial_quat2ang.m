...
%         end
        ...
        if(count == 5 && temp(count) == sensor_num)
            ...
            % Z-Y-X rotation
            ang(1) = atan2( 2*(q(2)*q(3)+q(1)*q(4)) , 1-2*(q(1)*q(1)+q(2)*q(2)) ) * 57.2975;
            ang(2) = asin ( 2*(q(2)*q(4)-q(1)*q(3)) ) * 57.2975;
            ang(3) = atan2( 2*(q(1)*q(2)+q(3)*q(4)) , 1-2*(q(2)*q(2)+q(3)*q(3)) ) * 57.2975;
        end
        ang
    end
